...
export default function ContestScreen() {
  ...
  const mutation = useMutation(
    (data: Record<string, any>) => {
      return axiosinstance.post(
        `${registerContestUrl}?contestId=${data?.contestId}`
      );
    },
    {
      onSuccess: () => {
        toast.success("Muvaffaqiyatli ro'yxatdan o'tdingiz");
        router.push(`${pathname}/contest?tab=1&id=${contest}`);
      },
      onError: (error: AxiosError) => {
        if (error?.response?.status === 404) {
          toast.error('Kontest mavjud emas');
        }
        if (error?.response?.status === 400) {
          toast.error("Siz ro'yxatdan o'tgansiz");
        }
      },
    }
  );
  ...
  const saveContestMutation = useMutation(
    ...
    },
    {
      ...
      onError: (error: AxiosError) => {
        if (error?.response?.status === 400) {
          toast.error('Musobaqa allaqachon saqlangan');
        }
      },
    }
  );
      ...
      <div className='w-full h-max flex flex-col justify-start items-start gap-5 overflow-y-scroll scrollbar-none'>
        ...
        {data?.data?.data?.map((item: Record<string, any>, index: number) => {
          return (
            <Link
              key={index}
              href={`${pathname}/contest?id=${item?.id}`}
              ...
              {item?.status !== 'Finished' ? (
                <button
                  ...
                  onClick={(e) => {
                    ...
                    e.preventDefault();
                    setContest(`${item?.id}`);
                    mutation.mutate({ contestId: item?.id });
                  }}
                  type='submit'
