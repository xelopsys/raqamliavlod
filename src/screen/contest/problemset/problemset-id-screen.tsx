...
export default function ProblemSetsTable({ problemId }: { problemId: string }) {
  ...
  const { generalProblemsetSubRowCol } = Headers();
    ...
    },
    {
      onSuccess: () => {
        queryClient.invalidateQueries({
          queryKey: [`problemset-id`, problemId],
        });
        refetch();
  ...
  return (
    <>
      {isFetching ? (
        ...
        <div className='min-w-fit w-full h-full flex flex-col justify-start items-center gap-y-5 box-border  break-words overflow-y-scroll scrollbar-thin scrollbar-track-slate-300 scrollbar-thumb-slate-600 bg-white rounded-lg p-8 border'>
          ...
          {isAuthenticated ? (
            <div className='w-full h-max flex flex-col justify-center items-start '>
              <form
                ...
                <section className='w-full flex flex-col flex-nowrap md:flex-row lg:flex-row justify-center items-center gap-4'>
                  ...
                  <label
                    ...
                    <span
                      className={classNames(
                        'px-4 text-sm md:text-lg lg:text-lg',
                  ...
                  <button
                    type='submit'
                    disabled={!file?.[0]}
                    className='w-full p-2 rounded-md border-2 border-purple-500 text-purple-500 hover:bg-purple-500 hover:text-white disabled:opacity-20 disabled:cursor-not-allowed'>
                    {mutation?.isLoading ? (
                      <>
                        <svg
                          aria-hidden='true'
              ...
              <PaginatedTable
                isPaginated={problems?.data?.['length'] > 5 || false}
                columns={generalProblemsetSubRowCol}
