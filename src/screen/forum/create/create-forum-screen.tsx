      ...
      {isAuthenticated ? (
        <div className='w-full lg:w-2/3 h-full flex flex-col justify-start items-center box-border gap-5 overflow-y-scroll scrollbar-none bg-white p-5 rounded-lg'>
          <div className='w-full flex flex-row justify-start items-start'>
            <div className='w-full lg:w-full flex flex-row justify-start items-center flex-wrap gap-3'>
              {tagsData?.map((tag: TObject, index: number) => {
                return (
                  <span
                    key={index}
                    className={classNames(
                      'py-1 px-2 rounded-md text-sm md:text-md lg:text-lg flex justify-center items-center cursor-pointer border',
                      activeTags?.find((item) => item?.id === tag?.id)
                        ? 'bg-purple-500 text-white'
                        : 'hover:border-none hover:bg-purple-500 hover:text-white'
                    )}
              ...
              <button
                ...
                className='py-1 px-2 rounded-md border text-sm md:text-md lg:text-lg flex justify-center items-center hover:border-none hover:bg-purple-500 hover:text-white cursor-pointer'>
                {showMore ? '-' : '+'}
              </button>
            </div>
          </div>
          <form
            ...
            className='w-full h-full flex flex-col justify-start items-start gap-y-3'>
            <div className='w-full h-full flex flex-col justify-start items-start gap-y-3'>
              ...
              <div className='w-full h-1/2 md:h-full lg:h-full flex flex-col justify-start items-start rounded-b-md relative z-0'>
                <h1 className='text-lg md:text-lg lg:text-lg text-gray-600 px-3'></h1>
                <TextArea handleSubmitText={handleText} />
              </div>
            </div>
            <button
              disabled={mutation.isLoading || !text}
              className='w-full rounded-lg py-2 px-3 bg-purple-500 text-white disabled:opacity-20 disabled:cursor-not-allowed hover:bg-white hover:text-purple-500 hover:ring-2 hover:ring-purple-500 sticky bottom-0 z-50'>
              {mutation.isLoading ? (
                <>
                  <svg
                    aria-hidden='true'
        ...
        <div className='w-full h-full flex flex-row justify-center items-center'>
          <h1 className='text-sm md:text-md lg:text-lg text-center'>
            Savol so{"'"}rash uchun siz ro{'`'}yxatdan o{'`'}tishingiz kerak
            yoki tizimga kirishingiz kerak
          </h1>
        </div>
      )}
    </>
