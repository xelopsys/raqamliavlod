...
import VideoComponent from '../../courses/video-screen';
import { classNames, compareDateAndGetLastSeen } from '@/helper';
import { AxiosError } from 'axios';
import UpdateCourseVideo from '@/form/update-course-video-modal';
import CreateCourseVideoModal from '@/form/create-course-video-modal';
...
export default function CoursesScreen({
  ...
  const deleteUrl = '/api/courses/videos';
  ...
  const deleteCommentUrl = `/api/courses/comments`;
  ...
  const deleteMutation = useMutation(
    (id: string) => {
      return axiosinstance.delete(`${apiUrl}`, { params: { id } });
    },
    {
      onSuccess: (data) => {
  ...
  const deleteCommentMutation = useMutation(
    (id: string) => {
      return axiosinstance.delete(`${deleteCommentUrl}/${id}`);
    },
    {
      onSuccess: (data) => {
  ...
  const handleEdit = (item: Record<string, any>) => {
    NiceModal.show(UpdateCourseVideo, {
      id: item?.id,
      link: item?.url,
    });
  };
        ...
        <div
          ...
          <div
            ...
            {isCollapsed ? (
              ...
              <h1 className='w-full text-sm md:text-lg lg:text-lg font-semibold px-10 flex justiyf-between items-center'>
                Kurs dasturi
                <PlusIcon
                  width={20}
                  className='ml-auto'
                  onClick={() => NiceModal.show(CreateCourseVideoModal, { id })}
                />
              </h1>
            )}
            {data?.data?.data?.map(
                        ...
                        (comment: Record<string, any>, index: number) => (
                          <>
                            <section className='w-full md:py-2 lg:py-2 md:px-3 lg:px-3'>
                              <p className='w-full text-sm md:text-md lg:text-lg flex flex-row justify-between'>
                                {comment?.text}
                                <TrashIcon
                                  width={20}
                                  onClick={() =>
