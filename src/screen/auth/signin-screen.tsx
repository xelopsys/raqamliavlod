...
export default function SignInScreen({ email }: { email: string }) {
  const apiUrl = '/api/users/login';
  const dataUrl = '/api/users/info/self';
  const { login } = useAuth();
  ...
  const getData = useMutation(
    (token: string) => {
      return axiosinstance.get(dataUrl, {
        headers: {
  ...
  const mutation = useMutation(
    ...
    },
    {
      onSuccess: (data) => {
        ...
        getData.mutate(data?.data?.token || '');
    ...
    <div className='w-full h-full flex flex-col justify-center items-center md:p-10 lg:p-14 overflow-scroll scrollbar-none gap-4'>
      <form
        ...
        <Input
          ...
          register={register('password', { required: true, value: email })}
          Icon={LockClosedIcon}
        />
        {/* <section>
          <Image
      ...
      <section className='w-full flex flex-row justify-center items-center gap-2'>
        ...
        <Link
          href='/signup'
          className='text-blue hover:text-blue-500 transition-all underline ml'>
          <p className='text-sm md:text-base lg:text-lg '>
            Ro{"'"}yxatdan o{"'"}ting
          </p>
        </Link>
